...
try {
  ...
  assert.match(overviewText, /Danach voraussichtlich 261,32\s*€ frei/);
  assert.match(overviewText, /120,00\s*€ mehr pro Monat/);
  ...
  assert.match(debtText, /Ablösesumme heute[\s\S]*12\.825,67\s*€/);
  assert.match(debtText, /Noch planmäßig zu zahlen[\s\S]*15\.480,00\s*€/);
  assert.match(debtText, /70 verbleibende Raten/);
  assert.match(debtText, /Zukünftige Mehrkosten[\s\S]*2\.654,33\s*€/);
  ...
  assert.match(await mobile.page.locator('.debt-progress').innerText(), /August 2031[\s\S]*0,00\s*€/);
